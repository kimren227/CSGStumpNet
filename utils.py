...
def generate_mesh(model, surface_point_cloud, config, test_iter, iso_value=0.5):
    ...
    complement_layer_weights, intersection_layer_connections, union_layer_connections = model.connection_head(code, is_training=False)
    primitive_parameters = model.primitive_head(code)
    occ_func = lambda sample_points: (model.csg_stump(sample_points, primitive_parameters, complement_layer_weights, intersection_layer_connections, union_layer_connections, is_training=False)[0]).detach().cpu().numpy()
